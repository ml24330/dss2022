...
def QueryAPI(site_id, keywords, table):
    try:
        api = Connection(appid=os.environ.get("APP_ID"), siteid=site_id, config_file=None)
        ...
        for num in range(1,min(101, int(page_count)+1)):
            ...
            for idx, item in enumerate(response.reply.searchResult.item):
                ...
                returnsAccepted = (1 if item.returnsAccepted == 'true' else 0) if hasattr(item, "returnsAccepted") else -1
            ...
            InsertListings(table, listings)
            print(f'database {table} now has {len(QueryAll(table))} entries')
